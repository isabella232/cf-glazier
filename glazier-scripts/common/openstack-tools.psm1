...
function Set-SystemProxy{[CmdletBinding()]param()
  ...
  $proxyString = "http=${httpProxy};https=${httpProxy}"
  Set-ItemProperty -path 'hklm:Software\Policies\Microsoft\Windows\CurrentVersion\Internet Settings' -name 'ProxySettingsPerUser' -value 0 -type dword
  Set-ItemProperty -path 'hklm:Software\Microsoft\Windows\CurrentVersion\Internet Settings' -name 'ProxyServer' -value $proxyString -type string
  Set-ItemProperty -path 'hklm:Software\Microsoft\Windows\CurrentVersion\Internet Settings' -name 'MigrateProxy' -value 1 -type dword
  ...
  Set-ItemProperty -path 'hklm:Software\Microsoft\Windows\CurrentVersion\Internet Settings' -name 'ProxyHttp1.1' -value 0 -type dword
  Set-ItemProperty -path 'hklm:Software\Microsoft\Windows\CurrentVersion\Internet Settings' -name 'ProxyOverride' -value '<local>' -type string`

  . "${env:windir}\regedit.exe" /s A:\pac.reg
  . "${env:windir}\syswow64\regedit.exe" /s A:\pac.reg
  
  $winHTTPProxyBits = @("", "")

  if (![string]::IsNullOrWhitespace($httpProxy)) {
    $winHTTPProxyBits[0] = "http=${httpProxy}"
  }
  if (![string]::IsNullOrWhitespace($httpsProxy)) {
    $winHTTPProxyBits[1] = "https=${httpsProxy}"
  }
  
  $bypassList = @("<local>", "0.*", "10.*", "127.*", "169.254.*", "172.16.*", "172.17.*",
    "172.18.*", "172.19.*", "172.20.*", "172.21.*", "172.22.*", "172.23.*",
    "172.24.*", "172.25.*", "172.26.*", "172.27.*", "172.28.*", "172.29.*",
    "172.30.*", "172.31.*", "192.0.2.*", "192.88.99.*", "192.168.*",
    "198.18.*", "198.19.*", "224.*", "225.*", "226.*", "227.*", "228.*", "229.*",
    "230.*", "231.*", "232.*", "233.*", "234.*", "235.*", "236.*", "237.*",
    "238.*", "239.*", "240.*", "241.*", "242.*", "243.*", "244.*", "245.*", "246.*",
    "247.*", "248.*", "249.*", "250.*", "251.*", "252.*", "253.*", "254.*", "255.*")
	
  $winHTTPProxyServer = $winHTTPProxyBits -join ';'
  $winHTTPBypassList = $bypassList -join ';'
  
  if( $winHTTPProxyServer -ne ';' ) {
    . "${env:windir}\system32\netsh.exe" winhttp set proxy proxy-server="${winHTTPProxyServer}" bypass-list="${winHTTPBypassList}"
	. "${env:windir}\syswow64\netsh.exe" winhttp set proxy proxy-server="${winHTTPProxyServer}" bypass-list="${winHTTPBypassList}"
  }
